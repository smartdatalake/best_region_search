...
object NstepAlgo {
  ...
  def localAlgo(input: (Int, Iterable[org.apache.spark.sql.Row]), finalAnswers: List[Row]): (Int, Iterable[org.apache.spark.sql.Row]) = {
    input;
  }
  ...
  def main(args: Array[String]) {
    ...
    while (finalAnswers.length <= requiredResults) {
      ...
      val resultGroupedPerNode = nodeToPoint.groupByKey().map(x => localAlgo(x, finalAnswers));
